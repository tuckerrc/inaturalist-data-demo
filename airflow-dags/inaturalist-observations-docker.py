...
with DAG(
    "inaturalist-observations-docker",
    default_args=default_args,
    description="Download Observations from iNaturalist",
    schedule_interval=timedelta(days=1),
    start_date=days_ago(2),
    catchup=False,
    tags=["inaturalist", "dbt", "cloudquery", "docker"],
) as dag:
    ...
    dbt_image = "ghcr.io/dbt-labs/dbt-postgres:1.7.5"

    dbt_mounts = [
        Mount(target="/usr/app", source=dbt_project_path, type="bind"),
        Mount(target="/root/.dbt/", source=dbt_profiles_dir, type="bind"),
    ]
